...
kubectl describe pod checkout-service-577dc59b8d| grep -i oom
```
<img width="1074" height="406" alt="Screenshot 2026-06-08 at 16 33 14" src="https://github.com/user-attachments/assets/766b532e-8a0c-418d-affc-7bd0e2ef3d95" />
...
7. validation pass (validate either via Grafana or curl /health or kubectl pods status)
...
## Investigation Steps
1. Grafana dashboard showed memory utilization growing linearly over time.
2. Kubernetes metrics revealed increasing container memory consumption.
3. Pod restart count increased unexpectedly.
4. kubectl describe pod confirmed OOMKilled events.
5. Jaeger traces remained healthy, eliminating downstream latency as the primary cause.
6. Log analysis indicated objects were retained in memory and not released.
7. Root cause identified as application memory leak.
